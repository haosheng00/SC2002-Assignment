        ...
        do{
            ...
            if (cineplexChoice > 0 && cineplexChoice < exit){
                ...
                if (cineplexChoice == selectedCineplexes.get(j)){
                    System.out.println("Cineplex " + cineplexes.get(cineplexChoice).getCineplexName() + "already added");
                    alreadySelected = true;
                }
            }
            if (alreadySelected){
                if (cineplexChoice > 0 && cineplexChoice < exit){
                    System.out.println("Added to Cineplex " + cineplexes.get(cineplexChoice).getCineplexName() + " successfully");
        ...
        do {
            ...
            for (i = 0; i < screenings.size(); i++) {
                ...
                if (movieTitle.equals(screenings.get(i).getMovie().getMovieTitle())){
                    optionNo++;
                    System.out.println("(" + optionNo + ")" + screenings.get(i).getShowDate() + " " + screenings.get(i).getShowTime());
                }
                else{
                    counter++;
                }
    ...
    public static int initiateMovieChoice(Cineplex cineplex, int haveComingSoon) {
        ...
        do {
            ...
            do {
                ...
                System.out.println("Select Movie: ");
                for (i = 0; i < cineplex.getMovies().size(); i++) {
                    if (cineplex.getMovies().get(i).getMovieStatus() == Enum.MovieStatus.END_OF_SHOWING || cineplex.getMovies().get(i).getMovieStatus() == Enum.MovieStatus.COMING_SOON){
                        counter++;
                    }
                    else {
                        optionNo = i + 1 - counter;
